...
contract WakandaPair is WKDLP {
    ...
    modifier lock() {
        require(unlocked == 1, "Wakanda: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "Wakanda: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, "Wakanda: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
        ...
        returns (uint amount0, uint amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "Wakanda: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "Wakanda: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "Wakanda: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint balance1;
        {
            ...
            require(to != _token0 && to != _token1, "Wakanda: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                IWakandaCallee(to).wakandaCall(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(
            ...
            "Wakanda: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
